...
def dnsdumpster(target):
    ...
    try:
        ...
        print_out(Fore.RED + "Error: " + str(e) + '\nTraceback:\n' + traceback.format_exc())
...
def subdomain_scan(target, subdomains):
    ...
    try:
        ...
        print_out(Fore.RED + "Runtime ERRROR or Subdomains file does not exist in data directory, aborting scan...")
...
def cloudfail_no_tor(target = None):
    ...
    status = init(args.target)

    if status is None:

        # Scan DNSdumpster.com
        try:
            dns_records = dnsdumpster(args.target)
        except Exception:
            dns_records = None

        # Scan Crimeflare database
        try:
            crimeflare_records = crimeflare(args.target)
        except Exception:
            crimeflare_records = None

        # FIXME: Subdomains file does not exist in data directory, aborting scan...
        # Scan subdomains with or without TOR
        subdomain_scan(args.target, args.subdomains)

        return dns_records, crimeflare_records
...
parser.set_defaults(subdomains=None)

args = SimpleNamespace(**parser._defaults)
